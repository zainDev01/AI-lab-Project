...
from simulated import GraphSimulatedAnnealing
...
class Ui_AISearchingTechniquesMainWindow(object):
    ...
    graphbestD = Graph_bestfirst(directed=True)
    graphsimu = GraphSimulatedAnnealing(directed=False)
        ...
        with open('test.txt', 'w') as f:
            ...
            if (graphType == "Undirectd Graph"):
                ...
                if searchType == "BFS":
                    ...
                    for i in range(0, self.counter):
                        self.graphdfs.add_edge(Node1_arr[i], Node2_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path2, goal = self.graphdfs.dfs(start, goals)
                    if (traced_path2):
                        print('Path:', end=' ')
                    ...
                    for i in range(0, self.counter):
                        self.g.add_edge(Node1_arr[i], Node2_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    print(self.g.bidirectional(start,goals))


                elif searchType == "Iterative deepning":
                    graphidfs = Graph_idfs(directed=False)
                    for i in range(0, self.counter):
                        graphidfs.add_edge(Node1_arr[i], Node2_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path8, goal = graphidfs.iterative_deepening(start, goals)
                    if (traced_path8):
                        print('Path:', end=' ')
                        Graph_idfs.print_path(traced_path8, goal)


                elif searchType == "Depth Limited":
                        graphidfs2 = Graph_idfs(directed=False)
                        for i in range(0, self.counter):
                            graphidfs2.add_edge(Node1_arr[i], Node2_arr[i])
                        start = self.StartNode_input.text()
                        goals = Goal_list
                        traced_path8, goal = graphidfs2.depth_limited_search(start, goals,3)
                        if (traced_path8):
                            print('Path:', end=' ')
                            Graph_idfs.print_path(traced_path8, goal)


                elif searchType == "SimulatedAnneling":

                        for i in range(0, self.counter):
                            self.graphsimu.add_edge(Node1_arr[i], Node2_arr[i])
                        start = self.StartNode_input.text()
                        goals = Goal_list
                        a = self.graphsimu.simulated_annealing(start, goals,1.0, 0.95)
                        print(a)
                        # if (traced_path8):
                        #     print('Path:', end=' ')
                        #     GraphSimulatedAnnealing.print_path(traced_path8)
                ...
                if searchType == "BFS":
                    ...
                    for i in range(0, self.counter):
                        self.graphbestD.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path4, cost2, goal = self.graphbestD.best_first_search(start, goals)
                    if (traced_path4):
                        print('Path:', end=' ')
                        Graph_bestfirst.print_path(traced_path4, goal)
                        print("\nCost :", cost2)
                    ...
                    gprahidfs = Graph_idfs(directed=True)
                    for i in range(0, self.counter):
                        gprahidfs.add_edge(Node1_arr[i], Node2_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    print(Graph_idfs.bidirectional_search(start, goals))
                    # if (traced_path7):
                    #     print('Path:', end=' ')
                    #     Graph_idfs.print_path(traced_path7, goal)


                elif searchType == "Depth Limited":
                    graphidfs2 = Graph_idfs(directed=True)
                    for i in range(0, self.counter):
                        graphidfs2.add_edge(Node1_arr[i], Node2_arr[i])
                    start = self.StartNode_input.text()
                    goals = Goal_list
